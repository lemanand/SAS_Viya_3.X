...
6. **Execute R code in Jupyter Notebook** (Below Info Doesn't Work on 2021.04.20 Installation Try)

   6.1 Install [R-irkernel](https://anaconda.org/r/r-irkernel)

   ```bash
   # Install R-ikernel
   conda install -c r r-irkernel
   # Verify R-ikernel
   conda list r-irkernel
   ```

   6.2 Install [R and essential packages](https://docs.anaconda.com/anaconda/packages/r-language-pkg-docs/#o-packages)

   ```bash
   conda install -c r r-essentials
   conda list r-essentials
   ```

   6.3 Add PATH to R (If Needed)
   ...
   6.4 Install prerequisite R packages for SAS R-SWAT

   ```bash
   conda install -c r r-httr
   # Below packages if needed
   conda install -c r r-dplyr
   conda install -c r r-jsonlite
   ```

   6.5 Install [SAS R-SWAT](https://github.com/sassoftware/R-swat/releases)
   ```bash
   R CMD INSTALL R-swat-1.6.0.9000+vb20060-linux-64.tar.gz
   ```

   6.6 Verify SAS R-SWAT
